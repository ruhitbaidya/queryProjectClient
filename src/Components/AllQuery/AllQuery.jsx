...
const AllQuery = () => {
  ...
  useEffect(() => {
    axios
      .get("https://query-project-server.vercel.app/findProduct", { withCredentials: true })
      .then((res) => {
  ...
  const handelSearch = (e)=>{
      ...
      axios
      .get(`https://query-project-server.vercel.app/searchproduct/${text}`, { withCredentials: true })
      .then((res) => {
    ...
    <div className="container mx-auto px-[20px] my-[50px]">
      <div className="my-[70px] grid grid-cols-1 lg:grid-cols-2 gap-[30px]">
        ...
        <div className="items-center justify-center hidden lg:flex">
          ...
          <button
            onClick={() => handelGridChange("2")}
            className="bg-blue-500 dark:bg-gray-100 text-white px-[30px] py-[12px] mr-[5px]"
          >
            <BsGrid />
          </button>
          <button
            onClick={() => handelGridChange("3")}
            className="bg-blue-500 dark:bg-gray-100 text-white px-[30px] py-[12px] mr-[5px]"
          >
            <BsGrid3X3 />
          </button>
        ...
        {product.length == 0 ? (
          ...
          product.map((item) => {
            return (
              <div
                key={item._id}
                className="rounded card  shadow-lg p-[15px] border border-gray-400 flex-col justify-between gap-[25px]"
              >
                <img
                ...
                <div className="flex justify-between items-center">
                  ...
                  <div>
                    <p className="mr-[20px]">${item.price}</p>
                  </div>
                </div>
                <div className="flex gap-[25px]">
                  <Link className="flex-1" to={`/queryDetails/${item._id}`}>
                  ...
                  <Link className="flex-1" to={``}>
                    <button
                      className="w-full border dark:border-gray-200 dark:bg-gray-800 dark:text-gray-800 py-[10px]"
                    >
                      Add Card
                    </button>
                  </Link>
